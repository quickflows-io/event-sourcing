...
use crate::event::BankAccountEvent::Withdraw;
...
#[derive(Debug, Clone)]
pub(crate) struct WithdrawCommandHandler<ES>
where
    ES: EventStore,
{
...
impl<ES> CommandHandler<WithdrawCommand> for WithdrawCommandHandler<ES>
where
    ES: EventStore,
{
    async fn handle(&self, command: WithdrawCommand) -> Result<(), Box<dyn Error + Send + Sync>> {
